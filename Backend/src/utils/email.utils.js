...
const createTransporter = () => {
  try {
      ...
      if (process.env.NODE_ENV === 'production' && process.env.EMAIL_USER && process.env.EMAIL_PASSWORD) {
          // ✅ PRODUCTION - Use Gmail with better config
          console.log('📧 Creating PRODUCTION Gmail transporter');
          ...
          // Development - Use Ethereal for testing
          console.log('📧 Creating DEVELOPMENT Ethereal transporter');
          return nodemailer.createTransport({
              host: 'smtp.ethereal.email',
              port: 587,
              auth: {
...
export const sendPasswordResetEmail = async (email, resetToken, userName) => {
  ...
  try {
      ...
      // Email HTML template (keep your existing HTML)
      const htmlContent = `
    <!DOCTYPE html>
    <html>
    <head>
    ...
    <body>
      <div class="email-container">
        ...
        <div class="content">
          ...
          <div class="security-notice">
            <p><strong>🔒 Security Notice:</strong> If you didn't request this password reset, please ignore this email.</p>
          </div>
        </div>
        <div class="footer">
          <p><strong>InvestBeans</strong></p>
          <p>© 2025 InvestBeans. All rights reserved.</p>
        </div>
      </div>
    </body>
    </html>
...
© 2025 InvestBeans. All rights reserved.
  `;
      ...
      // All retries failed
      console.error('❌ All email attempts failed after', MAX_RETRIES, 'tries');
